...
describe('HltbClient', () => {
  it('uses init → find authentication and converts seconds to minutes', async () => {
    ...
    expect(result).toMatchObject({ mainStory: 2400, mainPlusExtras: 3420, completionist: 3480 });
    expect(result?.completionist).not.toBe(276);
    expect(fetchMock.mock.calls[0]?.[0]).toContain('/api/find/init?t=1000');
    const search = fetchMock.mock.calls[1];
    expect(search?.[0]).toBe('https://howlongtobeat.com/api/find');
